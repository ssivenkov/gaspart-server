...
describe('Shopping Cart Controller', () => {
  ...
  it('should add cart items', async () => {
    ...
    expect(response.body.find((item) => item.partId === partId)).toEqual(
  ...
  it('should get updated cart items', async () => {
    const count = 2;
    ...
    const response = await request(app.getHttpServer())
      .patch(`/shopping-cart/count/${partId}`)
      .send({ count })
      .set('Cookie', login.headers['set-cookie']);
  ...
  it('should get updated total price of cart item', async () => {
    ...
    const response = await request(app.getHttpServer())
      .patch(`/shopping-cart/total-price/${partId}`)
  ...
  it('should delete cart item', async () => {
    ...
    await request(app.getHttpServer())
      .delete(`/shopping-cart/one/${partId}`)
      .set('Cookie', login.headers['set-cookie']);
    ...
    expect(
      response.body.find((item) => item.partId === partId),
    ).toBeUndefined();
  });
